    ...
    return (
        <nav className='relative z-10 px-4 sm:px-6 py-4 sm:py-6 flex items-center border-b border-white/10 bg-transparent w-full'>
            ...
            <div className='flex-1 flex justify-center min-w-0'>
                <button className='text-2xl font-bold select-none truncate' onClick={() => router.push('/')}>
                    <span className='text-[#CCFF00]'>{t('app.title')}</span>
                    <span className='text-white'>{t('app.titleHero')}</span>
                </button>
            </div>
            ...
            <div className='flex-1 flex justify-end items-center gap-2 min-w-0'>
                {showWishlist && (
                    ...
                    >
                        ...
                        <span className='hidden sm:inline'>{t('app.viewWishlist')}</span>
                    </button>
                )}
                {showSettings && (
                    <button
