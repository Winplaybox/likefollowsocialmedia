...
export default function Settings() {
  ...
  const handleSave = async () => {
    ...
    if (expectedLength && apiKey.length !== expectedLength) {
      setError(t('app.apiKeyInvalid', { model: model.charAt(0).toUpperCase() + model.slice(1), length: expectedLength }));
      return;
    }
  ...
  return (
    <AppContainer>
      ...
      <motion.div
        ...
        <div className="mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold mb-2">{t('app.model')} {t('app.settings')}</h1>
...
<div className="space-y-6 bg-[#0A0A0A] border border-white/10 rounded-2xl p-8">
          ...
          {saved && <div className="mt-4 p-3 bg-green-500/10 border border-green-500/20 rounded-lg text-green-400 text-sm text-center">{t('app.saved')}</div>}
        </div>
        </div></motion.div>
    </AppContainer>
  );
