...
interface GeneratedResponse {
 ...
 export default function CommentTool() {
  ...
  const handleGenerate = async () => {
    if (!context.trim()) {
      toast.error(t('commentTool.describeFirst'));
      return;
    }
    setLoading(true);
    try {
      ...
      if (response.status === 402) {
        toast.error(t('commentTool.apiKeyExceeded'));
      } else if (response.data.result) {
        setResult(response.data);
        toast.success(t('commentTool.generated'));
      } else if (response.data.error) {
        toast.error(response.data.error);
      }
    } catch (error) {
      console.error('Error:', error);
      toast.error(t('commentTool.failed'));
    } finally {
      setLoading(false);
    }
  };

  const comments = result.result ? result.result.split('---').map(c => c.trim()).filter(c => c) : [];
          ...
          {comments.length > 0 ? (
            comments.map((c, idx) => (
              <ResultDisplay
                key={idx}
                result={c}
                saveType={t('commentTool.saveType')}
                copyLabel={t('app.copy')}
                saveLabel={t('app.save')}
              />
            ))
          ) : (
            <ResultDisplay result="" placeholder={t('commentTool.resultPlaceholder')} icon={{type:'svg',path:'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z'}}/>
          )}
        {/* </div> */}
      </div>
    </div>
