...
export default function RemixTool() {
  ...
  const handleGenerate = async () => {
    if (!postContent.trim()) {
      toast.error(t('remixTool.enterContent'));
      return;
    }
    ...
    try {
      ...
      if (response.status === 402) {
        toast.error(t('remixTool.apiKeyExceeded'));
      } else if (response.data.result) {
        setResult(response.data.result);
        toast.success(t('remixTool.success'));
      } else if (response.data.error) {
        toast.error(response.data.error);
      }
    } catch (error) {
      console.error('Error:', error);
      toast.error(t('remixTool.failed'));
    } finally {
      setLoading(false);
    }
  };
  return (
    <div className="space-y-8">
          ...
          {PLATFORMS.map((platform) => {
            ...
            return (
              <div key={platform.value} className="bg-[#0A0A0A] border border-white/10 rounded-2xl p-6 space-y-4 flex flex-col">
                ...
                <ResultDisplay
                  result={content}
                  saveType={t('remixTool.saveType', { platform: platform.label })}
      ...
      <ResultDisplay result="" placeholder={t('remixTool.resultPlaceholder')} icon={{type:'svg',path:'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4'}}/>
      }
    </div>
  );
}