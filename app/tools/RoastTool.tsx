...
export default function RoastTool() {
  ...
  const handleGenerate = async () => {
    if (!profileDetails.trim()) {
      toast.error(t('roastTool.provideDetails'));
      return;
    }
    setLoading(true);
    try {
      ...
      if (response.status === 402) {
        toast.error(t('roastTool.apiKeyExceeded'));
      } else if (response.data.result) {
        setResult(response.data);
        toast.success(t('roastTool.success'));
      } else if (response.data.error) {
        toast.error(response.data.error);
      }
    } catch (error) {
      console.error('Error:', error);
      toast.error(t('roastTool.failed'));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div>
          ...
          {result.result ? (
            <ResultDisplay
              ...
              saveType={t('roastTool.saveType')}
              copyLabel={t('app.copy')}
              saveLabel={t('app.save')}
            />
          ) : (
            <ResultDisplay result="" placeholder={t('roastTool.resultPlaceholder')} icon={{type:'icon',path:<Flame className="w-12 h-12 text-[#A1A1AA]" />}}/>
          )}
      </div>
    </div>
  );
