...
export default function VisionTool() {
  ...
  const handleGenerate = async () => {
    if (!description.trim()) {
      toast.error(t('visionTool.describeFirst'));
      return;
    }
    setLoading(true);
    try {
      ...
      if (response.status === 402) {
        toast.error(t('visionTool.apiKeyExceeded'));
      } else if (response.data.result) {
        setResult(response.data);
        toast.success(t('visionTool.success'));
      } else if (response.data.error) {
        toast.error(response.data.error);
      }
    } catch (error) {
      console.error('Error:', error);
      toast.error(t('visionTool.failed'));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div>
          ...
          {result.result ? (
            <ResultDisplay
              ...
              saveType={t('visionTool.saveType')}
              copyLabel={t('app.copy')}
              saveLabel={t('app.save')}
            />
          ) : (
            <ResultDisplay result="" placeholder={t('visionTool.resultPlaceholder')} icon={{type:"icon",path:<Upload className="w-12 h-12 text-[#A1A1AA]" />}}/>
          )}
      </div>
    </div>
  );
