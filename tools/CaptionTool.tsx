...
export default function CaptionTool() {
    ...
    const handleGenerate = async () => {
        if (!description.trim()) {
            toast.error(t('commonTool.describeFirst', {tool: t('dashboard.caption'), context: 'the description for the caption'}));
            return;
        }
        setLoading(true);
        try {
            ...
            if (response.status === 402) {
                ...
                toast.success(t('commonTool.success', {tool: t('dashboard.caption')}));
    ...
    return (
        <>
            ...
            <View className='flex-1 gap-6  h-auto w-full'>
                <View>
                    <Text className='text-[28px] font-[450] mb-2 text-[#121317] tracking-tight'>{t('captionTool.title')}</Text>
                    <Text className='text-[#45474D] text-[17.5px] leading-relaxed'>{t('captionTool.description')}</Text>
                </View>
                <ToolForm
                    fields={[
                        {
            ...
            <View className='flex-1 gap-6  h-auto w-full'>
                <View>
                    <Text className='text-[28px] font-[450] mb-2 text-[#121317] tracking-tight'>{t('captionTool.generatedCaption')}</Text>
                    <Text className='text-[#45474D] text-[17.5px] leading-relaxed'>{t('captionTool.readyToPost')}</Text>
                </View>
                <ResultDisplay
